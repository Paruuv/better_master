...
class DataManager:
    ...
    """
    def __init__(self, lags: int = 20, train_size: float = 0.8, val_size: float = 0.1, test_size: float = 0.1, model_dtype = torch.float32):
        """
        lags : int
    ...
    def prepare(self):
        ...
        X_train = torch.tensor(lagged_sensor_measurements[self.train_indices], dtype=self.model_dtype, device=device)
        Y_train = torch.tensor(scaled_data[self.train_indices], dtype=self.model_dtype, device=device)

        X_val   = torch.tensor(lagged_sensor_measurements[self.val_indices], dtype=self.model_dtype, device=device)
        Y_val   = torch.tensor(scaled_data[self.val_indices], dtype=self.model_dtype, device=device)

        X_test  = torch.tensor(lagged_sensor_measurements[self.test_indices], dtype=self.model_dtype, device=device)
        Y_test  = torch.tensor(scaled_data[self.test_indices], dtype=self.model_dtype, device=device)
