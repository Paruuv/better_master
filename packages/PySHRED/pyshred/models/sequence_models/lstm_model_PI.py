...
class LSTM_PI(AbstractSequence):
    """
    ...
    def __init__(self, hidden_size:int =64, num_layers:int =2, layer_norm: bool = False):
        ...
        print("Yay LSTM_PI has been initialized")
    ...
    def forward(self, x):
        ...
        if self.use_layer_norm:
            ...
            h_last = out[:,-1,:]
        # Decode
        if self.decoder_type == "MLP" or self.decoder_type == "MLP_PI":
            h_third_last = h_third_last.view(-1,self.hidden_size)
            h_second_last = h_second_last.view(-1,self.hidden_size)
            h_last = h_last.view(-1, self.hidden_size)

            return h_third_last, h_second_last, h_last
        else:
            raise TypeError(
